...
namespace BubbleMain.Level
{
    ...
    public class Level4 : GameScene
    {
        ...
        private Game game;
        private int i, j, k, index, largepos, gunindex, savedcount;
        private double starttime, bubbleomega, score, redbubbleomega, pausedtime, resumetime, totalpausedtime = 0, happytime;
        public bool timecheck = true, endgame = false, stop = false, GameSaveRequested = false, savehighscore = true, paused = false, happy;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (!paused)
            {
                ...
                if (gameTime.TotalGameTime.TotalSeconds - starttime - totalpausedtime > bluestep && bluestep <= 17.5) //add blue bubbles
                {
                ...
                if (endgame && !stop)
                {
                    score = ((((Gun)actors[gunindex]).battery / 12f) * (saved / (float)BBLQTY) * (25f / totaltime) * 5);
                    if (score > 5)
                        score = 5;
            ...
            if ((GameSaveRequested) && (result.IsCompleted))
            {
                ...
                if (device != null && device.IsConnected)
                {
                    ...
                    if (container.FileExists(filename))
                    {
                        ...
                        if (data.score4 < score)
                        {
                            data.score4 = (float)score;
                        }
                        stream.Close();
                    }
                    else
