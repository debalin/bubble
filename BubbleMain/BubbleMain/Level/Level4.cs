...
namespace BubbleMain.Level
{
    ...
    public class Level4 : GameScene
    {
        private ContentManager Content;
        private GraphicsDeviceManager graphics;
        private Vector2 displayratio, boundaryposition, gunposition, bluebubbleposition, bubblesize, bluebubblevelocity, initsize, hole, blockposition, blocksize, redbubbleposition, redbubblevelocity;
        ...
        private double starttime, bubbleomega, score, redbubbleomega;
        public bool timecheck = true, endgame = false, stop = false;
        ...
        public override void Initialize()
        {
            ...
            scoretext = "Score : 0";
            score = 0;
            blockposition = new Vector2(250, 150);
            blocksize = new Vector2(150, 150);
            obstacle1 = new Rectangle[2];
            obstacle1[0] = new Rectangle(370, 150, 80, 80);
            obstacle1[1] = new Rectangle(370, 550, 80, 80);
            ...
            actors.Add(new DrawBackground(game, Content.Load<Texture2D>(@"Level\Level4\backlevel4_4_3"), Content.Load<Texture2D>(@"Level\Level4\backlevel4_4_3"), Content.Load<Texture2D>(@"Level\Level4\backlevel4_4_3")));
            ...
            actors.Add(new RedBubble(game, redbubbleposition, bubblesize, redbubblevelocity, new Vector2(890, 650), new Vector2(10, 50), redbubblemass, redbubbleomega, true, false, hole, obstacle1, 2, blockmass));
            ...
            actors.Add(new AnimateImage(game, Content.Load<Texture2D>(@"Core Elements\obstacle2_1"), Content.Load<Texture2D>(@"Core Elements\obstacle2_2"), new Vector2(obstacle1[1].X, obstacle1[1].Y), 10, Color.White, .6f, 5, 1));
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (gameTime.TotalGameTime.TotalSeconds - starttime > bluestep && bluestep <= 17.5) //add blue bubbles
            {
            ...
            for (i = gunindex + 1; i < actors.Count; i++) //check for collisions and respond accordingly
            {
                ...
                if (actors[i].GetType() == typeof(Bubble) && ((Bubble)actors[i]).draw)
                {
                    ...
                    if (((RedBubble)actors[10]).BubblesCollide((Bubble)actors[i])) //collision with red bubble
                    {
                        collisionangle = Math.Atan2((((Bubble)actors[i]).position.Y + ((Bubble)actors[i]).size.Y / 2) - (((RedBubble)actors[10]).position.Y + ((RedBubble)actors[10]).size.Y / 2), -(((Bubble)actors[i]).position.X + ((Bubble)actors[i]).size.X / 2) + (((RedBubble)actors[10]).position.X + ((RedBubble)actors[10]).size.X / 2));
                        direction1 = Math.Atan2(-((Bubble)actors[i]).velocity.Y, ((Bubble)actors[i]).velocity.X);
                        direction2 = Math.Atan2(-((RedBubble)actors[10]).velocity.Y, ((RedBubble)actors[10]).velocity.X);
                        magnitude1 = Math.Sqrt(((Bubble)actors[i]).velocity.X * ((Bubble)actors[i]).velocity.X + ((Bubble)actors[i]).velocity.Y * ((Bubble)actors[i]).velocity.Y);
                        magnitude2 = Math.Sqrt(((RedBubble)actors[10]).velocity.X * ((RedBubble)actors[10]).velocity.X + ((RedBubble)actors[10]).velocity.Y * ((RedBubble)actors[10]).velocity.Y);
                        tempvelx1 = magnitude1 * Math.Cos(direction1 - collisionangle);
                        tempvely1 = magnitude1 * Math.Sin(direction1 - collisionangle);
                        tempvelx2 = magnitude2 * Math.Cos(direction2 - collisionangle);
                        tempvely2 = magnitude2 * Math.Sin(direction2 - collisionangle);
                        tempvelx3 = ((((Bubble)actors[i]).mass - ((RedBubble)actors[10]).mass) * tempvelx1 / (((Bubble)actors[i]).mass + ((RedBubble)actors[10]).mass)) + (2 * ((RedBubble)actors[10]).mass * tempvelx2 / (((Bubble)actors[i]).mass + ((RedBubble)actors[10]).mass));
                        tempvelx4 = -((((Bubble)actors[i]).mass - ((RedBubble)actors[10]).mass) * tempvelx2 / (((Bubble)actors[i]).mass + ((RedBubble)actors[10]).mass)) + (2 * ((Bubble)actors[i]).mass * tempvelx1 / (((Bubble)actors[i]).mass + ((RedBubble)actors[10]).mass));
                        magnitude1 = Math.Sqrt(tempvelx3 * tempvelx3 + tempvely1 * tempvely1);
                        magnitude2 = Math.Sqrt(tempvelx4 * tempvelx4 + tempvely2 * tempvely2);
                        direction1 = Math.Atan2(tempvely1, tempvelx3) + collisionangle;
                        direction2 = Math.Atan2(tempvely2, tempvelx4) + collisionangle;
                        ((Bubble)actors[i]).omega = (tempvely2 - tempvely1) * 2 * 10 / (((Bubble)actors[i]).size.X * ((Bubble)actors[i]).mass);
                        ((RedBubble)actors[10]).omega = -(tempvely1 - tempvely2) * 2 * 10 / (((RedBubble)actors[10]).size.X * ((RedBubble)actors[10]).mass);
                        ((Bubble)actors[i]).velocity = new Vector2((float)(magnitude1 * Math.Cos(direction1)), -(float)(magnitude1 * Math.Sin(direction1)));
                        ((RedBubble)actors[10]).velocity = new Vector2((float)(magnitude2 * Math.Cos(direction2)), -(float)(magnitude2 * Math.Sin(direction2)));
                    }
                }
            ...
            ((AnimateImage)actors[11]).position = ((RedBubble)actors[10]).position;
            ((AnimateImage)actors[11]).rotation = ((RedBubble)actors[10]).rotation;

            if (endgame && !stop)
            {
                score = ((((Gun)actors[gunindex]).battery / 12f) * (saved / (float)BBLQTY) * (30f / totaltime) * 5);
                if (score > 5)
                    score = 5;
