...
namespace BubbleMain.Level
{
    ...
    public class Level2 : GameScene
    {
        ...
        private double starttime, bubbleomega, pausedtime, resumetime, totalpausedtime, happytime;
        public bool timecheck = true, endgame = false, GameSaveRequested = false, savehighscore = true, paused, happy;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (!paused)
            {
                ...
                if (gameTime.TotalGameTime.TotalSeconds - starttime - totalpausedtime > bluestep && bluestep <= 6.5) //add blue bubbles
                {
                ...
                if (endgame)
                {
                    score = ((((Gun)actors[gunindex]).battery / 12f) * (saved / (float)BBLQTY) * (18f / totaltime) * 5);
                    if (score > 5)
                        score = 5;
